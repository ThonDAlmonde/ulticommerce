...
class UltiCommerce_Webhook_Handler {

    public function __construct() {
        add_action( 'rest_api_init', [ $this, 'register_webhook_route' ] );
        add_filter( 'ulti_register_payment_gateways', [ $this, 'register_custom_webhook_gateway' ] );
    }

    public function register_webhook_route() {
        register_rest_route( 'wpc/v1', '/payment/confirm', [
            'methods'             => 'POST',
    ...
    public function handle_confirm( $request ) {
        ...
        if ( ! $order_id || ! get_post( $order_id ) || get_post_type( $order_id ) !== 'order' ) {
        ...
        do_action( 'ulti_order_status_changed', $order_id, $old_status, $new_status );
...
new UltiCommerce_Webhook_Handler();
