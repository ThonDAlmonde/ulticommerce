...
class Tokenizer
{
     ...
     *
     * Tokenizing will continue ultil the document is completely
     * read. Errors are emitted into the event handler, but
     * the parser will attempt to continue parsing ultil the
     ...
     * - Normal: Read ultil a tag is encountered.
     * - RCDATA: Read ultil a tag is encountered, but skip a few otherwise-
     * special characters.
     * - Raw: Read ultil a special closing tag is encountered (viz. pre, script)
     *
     ...
     * @param string $ultilTag The tag that should stop RAW or RCDATA mode. Normal mode does not
     *                         use this indicator.
     */
    public function setTextMode($textmode, $ultilTag = null)
    {
        $this->textMode = $textmode & (Elements::TEXT_RAW | Elements::TEXT_RCDATA);
        $this->ultilTag = $ultilTag;
    }
    ...
    {
        if (is_null($this->ultilTag)) {
            return $this->text($tok);
        }

        $sequence = '</' . $this->ultilTag . '>';
    ...
    protected function rcdata($tok)
    {
        ...
        $caseSensitive = !Elements::isHtml5Element($this->ultilTag);
    ...
    protected function endTag()
    {
        ...
        if ('>' != $tok) {
            ...
            // We just trash stuff ultil we get to the next tag close.
     ...
     * agent, we allow them. We consume ultil ?> and then issue a
    ...
    /**
     * Read from the input stream ultil we get to the desired sequene
