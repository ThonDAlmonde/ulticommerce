...
class UltiCommerce_Login_Widget extends WP_Widget {
    ...
    public function __construct() {
        parent::__construct(
            ...
            __( 'UltiCommerce Login', 'ulticommerce-core' ),
            [ 'description' => __( 'Display SSO login buttons and a login form.', 'ulticommerce-core' ) ]
        );
    }
    ...
    private function render_sso_buttons() {
        ...
        echo '</div>';
        echo '<div class="ulti-sso-divider"><span>' . esc_html__( 'OR', 'ulticommerce-core' ) . '</span></div>';
    }
    ...
    private function render_login_form() {
        ...
        <form method="post" action="<?php echo esc_url( wp_login_url() ); ?>" class="ulti-login-form">
            <p>
                <label for="ulti-user-login"><?php esc_html_e( 'Username or Email', 'ulticommerce-core' ); ?></label>
                <input type="text" name="log" id="ulti-user-login" required>
            </p>
            <p>
                <label for="ulti-user-pass"><?php esc_html_e( 'Password', 'ulticommerce-core' ); ?></label>
                <input type="password" name="pwd" id="ulti-user-pass" required>
            </p>
            <input type="hidden" name="redirect_to" value="<?php echo esc_attr( $redirect ); ?>">
            <p>
                <button type="submit"><?php esc_html_e( 'Sign In', 'ulticommerce-core' ); ?></button>
            </p>
        </form>
        <?php
    }
    ...
    public function form( $instance ) {
        $title = ! empty( $instance['title'] ) ? $instance['title'] : __( 'Login', 'ulticommerce-core' );
        ?>
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_attr_e( 'Title:', 'ulticommerce-core' ); ?></label>
